...
def lane_counts(tasks: List[Dict[str, Any]]) -> Dict[str, Dict[str, int]]:
    """Per-owner doing/blocked/superseded counts in a single pass."""
    counts: Dict[str, Dict[str, int]] = {}
    for task in tasks:
        # Lane ids are strings; a malformed non-scalar owner (e.g. a YAML
        # list) is unhashable and can never match a lane anyway.
        if task["column"] in ("doing", "blocked", "superseded") and isinstance(task["owner"], str) and task["owner"]:
            per = counts.setdefault(
                task["owner"], {"doing": 0, "blocked": 0, "superseded": 0})
            per[task["column"]] += 1
    return counts
